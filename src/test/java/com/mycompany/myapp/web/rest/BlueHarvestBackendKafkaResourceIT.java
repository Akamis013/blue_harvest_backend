...
class BlueHarvestBackendKafkaResourceIT {
    ...
    void setup() {
        ...
        BlueHarvestBackendKafkaResource kafkaResource = new BlueHarvestBackendKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/blue-harvest-backend-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc.perform(get("/api/blue-harvest-backend-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
